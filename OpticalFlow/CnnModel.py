...
class EncoderModel(nn.Module):
    def __init__(self):
        ...
        self.layer_channels = [6, 64, 96, 128, 256, 512]

        self.Conv0 = ConvBnActiv(in_channels=self.layer_channels[0], out_channels=self.layer_channels[1], kernel_size=3,
                                 padding=1)
...
class DecoderModel(nn.Module):
    def __init__(self):
        ...
        self.layer_channels = [512, 256, 128, 96, 64, 3]
        self.DilatedConv1 = ConvBnActiv(in_channels=self.layer_channels[0], out_channels=self.layer_channels[0], dilation_rate=2, padding=2)
        self.DilatedConv2 = ConvBnActiv(in_channels=self.layer_channels[0], out_channels=self.layer_channels[0], dilation_rate=4, padding=4)
        self.TransConv1 = TransposeConvBnActiv(in_channels=self.layer_channels[0], out_channels=self.layer_channels[1])
        self.ResLayer1 = ResidualBlock(in_channels=self.layer_channels[1], out_channels=self.layer_channels[1])
        self.ResLayer2 = ResidualBlock(in_channels=self.layer_channels[1], out_channels=self.layer_channels[1])
        self.TransConv2 = TransposeConvBnActiv(in_channels=self.layer_channels[1], out_channels=self.layer_channels[2])
        ...
        self.TransConv3 = TransposeConvBnActiv(in_channels=self.layer_channels[2], out_channels=self.layer_channels[3])
        ...
        self.TransConv4 = TransposeConvBnActiv(in_channels=self.layer_channels[3], out_channels=self.layer_channels[4])
        self.Conv7 = ConvBnActiv(in_channels=self.layer_channels[4], out_channels=self.layer_channels[4])
        self.Conv8 = ConvBnActiv(in_channels=self.layer_channels[4], out_channels=self.layer_channels[4])
        self.Conv9 = ConvBnActiv(in_channels=self.layer_channels[4], out_channels=self.layer_channels[5], kernel_size=1, stride=1, padding=0, apply_activation=False, apply_bias=False, apply_bn=False)
...
class FlowEstimationCnn(nn.Module):
    def __init__(self):
        super(FlowEstimationCnn, self).__init__()
