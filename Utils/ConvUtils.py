...
class TransposeConvBnActiv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel=3, stride=2, in_padding=(1, 1), out_padding=(1, 1),
                 apply_bn=True, apply_activation=True):
...
class ConvBnActiv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, groups=1, dilation_rate=1,
                 apply_bn=True, apply_activation=True, apply_bias=True, activ_type="silu"):
...
class DepthWiseConvResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, expansion_factor=6, stride=1, dilation_rate=1, apply_activ=False,
                 use_skip=True):
