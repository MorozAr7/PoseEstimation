...
def one_epoch(model, optimizer, dataloader, loss_functions, is_training=True, epoch=0):
	...
	if is_training:
		...
		return epoch_loss_l1_u_map / len(train_dataset), epoch_loss_l1_v_map / len(train_dataset), epoch_loss_l1_w_map / len(train_dataset), torch.sum(epoch_iou) / len(train_dataset)
	else:
		with torch.no_grad():
			for index, (image, mask, u_map, v_map, w_map) in enumerate(dataloader):
			...
			return epoch_loss_l1_u_map / len(validation_dataset), epoch_loss_l1_v_map / len(validation_dataset), epoch_loss_l1_w_map / len(validation_dataset), torch.sum(epoch_iou) / len(validation_dataset)
