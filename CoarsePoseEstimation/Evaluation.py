...
class CoarsePoseEvaluation:
    def __init__(self, device):
        ...
        self.distortion_coefficients = np.array(
            self.io.load_json_file(MAIN_DIR_PATH + "/CameraData/camera_data_1.json")["dist_coef"])
        self.device = device
    ...
    def solve_pnp_ransac(self, points3d, points2d):
        ...
        try:
            _, rvecs, tvecs, inliers = cv2.solvePnPRansac(np.array(points3d, dtype=float),
                                                          ...
                                                          iterationsCount=100, reprojectionError=1,
        ...
        for index in range(images.shape[0]):
            ...
            for i in range(uvw_array.shape[0]):
                try:
                    ...
                    points_2d.append([coords_x_masked[i] * scale_coefficient + bbox[0],
                                      coords_y_masked[i] * scale_coefficient + bbox[1]])
                except KeyError:
                    pass
            ...
            if index == 0:
                ...
                coarse_pose_predictions = np.concatenate([coarse_pose_predictions, predicted_transformation_matrix],
                                                         axis=0)
