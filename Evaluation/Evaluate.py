...
class Evaluation:
	...
	def crop_image(self, frame, bbox):
		...
		return frame[y_min:y_min + max_size, x_min:x_min + max_size]
	...
	def evaluate_video(self):
		...
		for index in range(num_frames):
			...
			cropepd_image = self.crop_image(frame, bbox)
			...
			#refined_pose_prediction = self.pose_refinement.get_refined_pose(np.expand_dims(cropepd_image, axis=0), np.expand_dims(pose_prediction, axis=0), bboxes=[bbox])
			#print(refined_pose_prediction[0, 0:3], pose_prediction[0:3, -1])
			...
			#img_dict_refined = self.dataset_renderer.get_image(transformation_matrix=pose_prediction, image_black=True, image_background=False, constant_light=True)
			#img_rendered_refined = img_dict_refined["ImageBlack"]
			#mask = np.expand_dims(img_dict_refined["Mask"], axis=-1)
			#visualize_rerfined = frame * (1 - mask) + img_rendered_refined
			cv2.imshow("video", visualize_coarse/255)
			cv2.waitKey(1)
