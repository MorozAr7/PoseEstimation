...
def one_epoch(pose_refiner_model, optimizer, dataloader, loss_function, is_training=True, epoch=0):
    ...
    if is_training:
        for index, (images_real, images_rendered, T_target, T_coarse) in enumerate(dataloader):
            ...
            visualize = 255 * torch.cat([images_real, images_rendered], dim=-1).permute(0, 2, 3,
                                                                                        1).detach().cpu().numpy()

            cv2.imwrite("image_test_11.png".format(epoch), visualize[0].astype(np.uint8))
            ...
            loss_R, loss_xy, loss_z, loss_total = loss_data["LossR"], loss_data["LossXY"], loss_data["LossZ"], \
                                                  loss_data["LossTotal"]
        ...
        return epoch_loss_rotation / len(train_dataset), epoch_loss_translation_xy / len(
            train_dataset), epoch_loss_translation_z / len(train_dataset)
    else:
        ...
        with torch.no_grad():
            ...
            return epoch_loss_rotation / len(validation_dataset), epoch_loss_translation_xy / len(
                validation_dataset), epoch_loss_translation_z / len(validation_dataset)
...
def main(pose_refiner_model, optimizer, training_dataloader, validation_dataloader) -> None:
    ...
    for epoch in range(0, NUM_EPOCHS):
        ...
        print("SAVING MODEL")
        torch.save(pose_refiner_model.state_dict(), "{}.pt".format(
            MAIN_DIR_PATH + "RefinedPoseEstimation/TrainedModels/RefinedPoseEstimationBigModel"))
...
if __name__ == "__main__":
    ...
    subset = "Training"
    train_dataset = Dataset(subset, NUM_DATA[subset], dataset_renderer,
                            PoseEstimationAugmentation if USE_AUGMENTATION else None)
    subset = "Validation"
    ...
    main(pose_refiner_model, optimizer, training_dataloader, validation_dataloader)
